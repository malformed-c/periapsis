...
// Covers the full lifecycle: start, run, exit.
// If ExitCode is set, the unit process exited and restart policy is applied.
func (f *Focus) handleUnitFact(fact *types.UnitFact) {
	// Use pre-parsed container name from the caller; fall back to parsing.
	...
	// Substate transition (always applied).
	...
	case "failed", "dead", "stop-sigterm":
		// Substate says the unit is gone. Apply exit logic below.
	default:
		return
	}
	...
	if fact.ExitCode != nil {
		...
		if !cv.seenRunning {
			f.logger.Debug("exit received but container never seen running - deferring",
				"container", containerName, "exitCode", *fact.ExitCode)
			cv.state = types.WaitingState("ContainerCreating")
			f.pushStatusLocked()
			return
		}
		...
		if shouldRestart {
			...
			f.logger.Info("container exited, restarting",
				"container", containerName, "exitCode", exitCode,
			...
			cv.state = types.TerminatedState(exitCode, finishedAt)
