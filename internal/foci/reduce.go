...
func Reduce(state PodState, fact types.Fact) (PodState, []types.Effect) {
	...
	default:
        ...
        case *types.PodRegisterFact:
                return reducePodRegisterFact(state, f)
        case *types.PodPromoteFact:
                return reducePodPromoteFact(state, f)
        case *types.PodDeleteFact:
                return reducePodDeleteFact(state, f)
        case *types.PodMarkDeletingFact:
                return reducePodMarkDeletingFact(state, f)
        default:
                return state, nil
        }
}
...
func reduceUnitFact(state PodState, fact *types.UnitFact) (PodState, []types.Effect) {
        ...
        case "running":
                ...
                // Reset probe timing state on container start.
                cv.ProbeStartedAt = time.Now()
                cv.LastProbeTime = make(map[string]time.Time)
                cv.StartupFailCount = 0
                cv.LiveFailCount = 0
                cv.ReadyFailCount = 0
                cv.ReadySuccessCount = 0
...
func reduceProbeFact(state PodState, fact *types.ProbeFact) (PodState, []types.Effect) {
        ...
        // Store probe timing counts from ProbeScheduler.
        cv.StartupFailCount = fact.StartupFailCount
        cv.LiveFailCount = fact.LiveFailCount
        cv.ReadyFailCount = fact.ReadyFailCount
        cv.ReadySuccessCount = fact.ReadySuccessCount

        // Record when this probe type was last executed.
        if cv.LastProbeTime == nil {
                cv.LastProbeTime = make(map[string]time.Time)
        }
        cv.LastProbeTime[fact.ProbeType] = time.Now()
...
func reducePodRegisterFact(state PodState, fact *types.PodRegisterFact) (PodState, []types.Effect) {
        if state.UID != "" {
                // Already tracking this pod - treat as spec update.
                return reduceSpecFact(state, types.NewSpecFact(fact.UID(), fact.Namespace, fact.Name, fact.Pod))
        }
...
func reducePodPromoteFact(state PodState, fact *types.PodPromoteFact) (PodState, []types.Effect) {
        if state.UID == "" {
                // Not tracking this pod - ignore.
                return state, nil
        }

        state.PodIP = fact.PodIP

        effects := []types.Effect{
                types.PromotePodRunning{
                        UID:   fact.UID(),
                        Pod:   fact.Pod,
                        PodIP: fact.PodIP,
                },
...
func reducePodDeleteFact(state PodState, fact *types.PodDeleteFact) (PodState, []types.Effect) {
        if state.UID == "" {
                // Not tracking this pod - just emit UnregisterPod.
                return PodState{}, []types.Effect{
                        types.MarkPodDeleting{UID: fact.UID()},
                        types.UnregisterPod{
                                UID:       fact.UID(),
                                Namespace: fact.Namespace,
                                Name:      fact.Name,
        ...
        // Emit ResetUnit for every tracked container.
        effects := make([]types.Effect, 0, len(state.Containers)+3)
        effects = append(effects, types.MarkPodDeleting{UID: fact.UID()})
        for _, cv := range state.Containers {
                effects = append(effects, types.ResetUnit{
                        UID:           state.UID,
                        ContainerName: cv.Name,
        ...
        effects = append(effects, types.UnregisterPod{
                UID:       fact.UID(),
                Namespace: fact.Namespace,
                Name:      fact.Name,
        })
...
func reducePodMarkDeletingFact(state PodState, fact *types.PodMarkDeletingFact) (PodState, []types.Effect) {
        if state.UID == "" {
                return state, nil
        }

        return state, []types.Effect{
                types.MarkPodDeleting{UID: fact.UID()},
        }
}
