...
type Runtime interface {
	...
	// GetContainerExitInfo returns detailed exit information for a container
	// that has reached a terminal state (Failed or Exited). Callers should
	// use this when MachineStatus returns StateFailed or StateExited to
	// enrich error messages with exit code, systemd Result, and the last
	// few lines of container output from journald.
	GetContainerExitInfo(ctx context.Context, podUID, containerName string) ContainerExitInfo
}
...
// ContainerExitInfo describes why a container exited or failed.
// Populated by Runtime.GetContainerExitInfo when a container reaches
// a terminal state (Failed or Exited).
type ContainerExitInfo struct {
	// ExitCode is the process exit code from systemd's ExecMainStatus.
	// For signal exits this is the signal number.
	ExitCode int32

	// Result is systemd's unit Result property: "success", "exit-code",
	// "signal", "core-dump", "timeout", "start-limit-hit", etc.
	Result string

	// LastLog contains the last few lines of container output captured
	// from journald. Empty if the journal is unavailable.
	LastLog string
}
