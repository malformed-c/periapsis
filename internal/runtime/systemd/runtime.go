...
func (s *SystemdRuntime) RunMachine(ctx context.Context, podUID string, cfg runtime.PodConfig) error {
	...
	userNS := cfg.RunAsUser != nil && !cfg.Privileged
	shimUserNS := userNS && usernsShimExists() && false
	...
	if userNS {
		// Use nspawn's userns isolation
		execStart = append(execStart, "--private-users=pick")
		execStart = append(execStart, fmt.Sprintf("--user=%d", *cfg.RunAsUser))

	} else if !shimUserNS && cfg.RunAsUser != nil {
		// Fallback: use nspawn's --user= (no userns isolation)
		s.logger.Error("UserNS isolation is requested, but no backends are available")
		execStart = append(execStart, fmt.Sprintf("--user=%d", *cfg.RunAsUser))
	}
	...
	if _, err := s.conn.StartTransientUnitContext(ctx, serviceName, "replace", properties, nil); err != nil {
		// Clean up PTY, FIFOs, and bind tmpdir on failure.
...
// GetContainerExitInfo returns detailed exit information for a container that
// has reached a terminal state. It reads the exit code, systemd Result
// property, and the last few lines of container output from journald.
// This should be called when MachineStatus returns StateFailed or StateExited
// to enrich error messages with actionable diagnostic information.
func (s *SystemdRuntime) GetContainerExitInfo(ctx context.Context, podUID, containerName string) runtime.ContainerExitInfo {
	serviceName := wrapperUnitName(s.pawnName, podUID, containerName)
	info := runtime.ContainerExitInfo{
		ExitCode: s.readExitCode(ctx, serviceName),
	}

	// Read systemd Result property: "exit-code", "signal", "timeout",
	// "core-dump", "start-limit-hit", etc.
	if prop, err := s.conn.GetServicePropertyContext(ctx, serviceName, "Result"); err == nil {
		if result, ok := prop.Value.Value().(string); ok {
			info.Result = strings.Trim(result, "\"")
		}
	}

	// Tail the last few lines from journald for this container.
	// This captures the actual error output (e.g. "Read-only file system")
	// that would otherwise require the operator to run journalctl manually.
	info.LastLog = s.tailContainerLog(ctx, podUID, containerName, 5)
...
func (s *SystemdRuntime) tailContainerLog(ctx context.Context, podUID, containerName string, lines int) string {
	...
	// Limit read to 4 KiB to avoid pulling huge log buffers.
	limited := io.LimitReader(rc, 4096)
