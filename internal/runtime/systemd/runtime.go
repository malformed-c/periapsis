...
// exited or failed. Delegates to readExitCode which reads ExecMainStatus
// from systemd. Returns 0 if the unit is still running or the property
// is unavailable.
func (s *SystemdRuntime) MachineExitCode(ctx context.Context, podUID, containerName string) int32 {
	serviceName := wrapperUnitName(s.pawnName, podUID, containerName)
	return s.readExitCode(ctx, serviceName)
}
