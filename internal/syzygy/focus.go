...
// It processes Facts and emits PodStatusFacts to Horizon.
type Focus struct {
	view    podView
...
func NewFocus(cfg FocusConfig) *Focus {
	...
	for _, c := range cfg.Pod.Spec.Containers {
		containers[c.Name] = &containerView{
			...
			ready:     cfg.Pod.ReadinessProbe == nil, // will be overridden by InitRestartState
...
func (f *Focus) handleUnitFact(fact *types.UnitFact) {
	uid, containerName := parseUnitName(fact.UnitName)
	if uid != f.view.uid {
		return
	}
	...
	if !ok {
		return // not an app container (or pod not tracked)
	}
...
func (f *Focus) handleProbeFact(fact *types.ProbeFact) {
	...
	case "startup":
		...
		// TODO: startup probe gate
	case "liveness":
		cv.probeLastResult["liveness"] = now
		// TODO: liveness restart
	}
...
// parseUnitName extracts uid and containerName from a systemd unit name.
// Format: perigeos-<pawn>-pod-<uid>-<containerName>.service
func parseUnitName(unitName string) (uid, containerName string) {
	// TODO: use actual parsing logic from gambit.ParseUnitName
	// For now, this is a placeholder that Syzygy's caller should use.
	return "", ""
}
