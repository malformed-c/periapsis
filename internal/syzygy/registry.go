...
type FocusRegistry struct {
	mu     sync.RWMutex
	foci   map[string]*Focus // uid → Focus
	logger *slog.Logger
}
...
func (r *FocusRegistry) Get(uid string) *Focus {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.foci[uid]
}
...
func (r *FocusRegistry) Create(ctx context.Context, cfg FocusConfig) (*Focus, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func factUID(fact *types.Fact) string {
	switch f := (*fact).(type) {
	case *types.UnitFact:
		uid, _ := parseUnitName(f.UnitName)
		return uid
	case *types.ExitFact:
		return f.UID
...
// timeUntilProbe returns the duration until the next probe should fire.
// Returns 0 if the probe should fire immediately.
func timeUntilProbe(cv *containerView, probeType string, periodSeconds, initialDelaySeconds int32) time.Duration {
	if periodSeconds <= 0 {
		periodSeconds = 10
	}

	last, ok := cv.probeLastResult[probeType]
	if !ok {
		// First probe: respect initial delay from container start.
		if initialDelaySeconds > 0 {
			elapsed := time.Since(cv.lastStarted)
			delay := time.Duration(initialDelaySeconds) * time.Second
			if elapsed >= delay {
				return 0
			}
