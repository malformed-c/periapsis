...
This is the primary concern. Periapsis solves a real problem in a non-obvious way. While the architecture is documented, the ADRs are public, the benchmarks are reproducible. A well-resourced team could reimplement it in a few months; The project should not be absorbed and erased by a party with more resources and distribution power.
...
Periapsis was developed with significant assistance from Claude. The architecture, design decisions, debugging, and implementation strategy were directed by the author; Claude served as a highly capable implementation tool in the same way a compiler or IDE does.
