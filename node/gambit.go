...
// writeResolvConf writes a cluster-DNS resolv.conf into the overlay rootfs.
//
// TODO(overlay-refactor): Remove once --overlay= is the sole path.
// resolv.conf is now generated by prepareBindFiles and bind-mounted via
// --bind-ro= so no merged rootfs needs to exist before container start.
