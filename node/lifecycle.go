...
func (g *Gambit) launchContainer(
	...
	cfg := perigeos.PodConfig{
		...
		RootFS:                        rootfs, // empty on mstack path, set on legacy path
...
// prepareUserIdentityForRootFS injects /etc/passwd and /etc/group entries
// into the merged rootfs for the legacy --directory= path (systemd < 260).
// On systemd >= 260, these files are injected via mstack robind@ entries.
//
// TODO(overlay-refactor): Remove once systemd 260 is ubiquitous.
func prepareUserIdentityForRootFS(rootfs string, runAsUser, runAsGroup *int64, logger *slog.Logger) {
	if runAsUser == nil {
		return
	}
	...
	passwdLine := fmt.Sprintf("%s:x:%d:%d::%s:/bin/sh\n", username, uid, gid, home)
	groupLine := fmt.Sprintf("peri-%d:x:%d:\n", gid, gid)

	passwdPath := filepath.Join(rootfs, "etc", "passwd")
	if f, err := os.OpenFile(passwdPath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644); err == nil {
	...
	for time.Now().Before(deadline) {
		...
		if err != nil {
			...
			case perigeos.StateFailed:
				...
				// (exit code, systemd Result, last log lines) so the error
				// message and k8s event are actionable without requiring the
				// operator to dig through journalctl manually.
				exitInfo := g.Runtime.GetContainerExitInfo(ctx, uid, containerName)
				detail := formatExitInfo(exitInfo)

				g.Logger.Warn("waitForContainer: container failed on startup",
					"uid", uid, "container", containerName,
					"exitCode", exitInfo.ExitCode, "result", exitInfo.Result)

				return fmt.Errorf("app container %s/%s failed on startup%s", uid, containerName, detail)
			}
		}
...
// The string is empty if there is nothing to report, or starts with " (" and
// ends with ")" so it can be directly appended to a base error message.
//
// Example outputs:
//
//	""
//	" (exit 1, exit-code)"
//	" (exit 1, exit-code): rm: cannot remove '/var/log/nginx/error.log': Read-only file system"
func formatExitInfo(info perigeos.ContainerExitInfo) string {
	if info.ExitCode == 0 && info.Result == "" && info.LastLog == "" {
		return ""
	}

	var parts []string
	if info.ExitCode != 0 {
		parts = append(parts, fmt.Sprintf("exit %d", info.ExitCode))
	}
	if info.Result != "" {
		parts = append(parts, info.Result)
	}
