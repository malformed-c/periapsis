...
func (g *Gambit) syncPodSandboxAndContainers(ctx context.Context, pod *corev1.Pod, pullCache map[string]pullCacheEntry) error {
        ...
        for i := range pod.Spec.InitContainers {
                ...
                machineState, _ := g.Runtime.MachineStatus(ctx, uid, ic.Name)
                ...
                if isSidecar && machineState == perigeos.StateRunning {
                        g.Logger.Debug("sync: step3 sidecar already running, skipping", "uid", uid, "pod", pod.Name, "container", ic.Name)

                        continue

                } else if !isSidecar && machineState == perigeos.StateExited {
        ...
        for i := range pod.Spec.Containers {
                ...
                if g.store.PodPhase(uid) == corev1.PodRunning {
                        ...
                        machineState, _ := g.Runtime.MachineStatus(ctx, uid, c.Name)

                        // If systemd already knows about it, skip. Let the BatchWatcher handle its lifecycle.
                        if machineState == perigeos.StateRunning ||
                                machineState == perigeos.StateExited ||
                                machineState == perigeos.StateFailed {
                                g.Logger.Debug("sync: step4 container already known to systemd, skipping", "uid", uid, "pod", pod.Name, "container", c.Name, "state", machineState)
...
func (g *Gambit) launchContainer(
        ...
        if isInit {
                ...
                // Emit MarkRunningFact so Syzygy's state machine knows this
                // container was observed running, even if the D-Bus "running"
                // signal arrives after the unit exits (fast-exit containers).
                if g.sendFact != nil {
                        g.sendFact(types.NewMarkRunningFact(uid, c.Name))
                }
