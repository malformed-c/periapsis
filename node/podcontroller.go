...
type PodController struct {
	...
	// Access and update atomically to avoid data races between sync workers.
...
func (pc *PodController) Run(ctx context.Context, podSyncWorkers int) (retErr error) {
	...
	var eventHandler cache.ResourceEventHandler = cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(oldObj, newObj any) {
			...
			if !ok {
				log.G(ctx).Error("UpdateFunc: oldObj is not a *corev1.Pod", "type", fmt.Sprintf("%T", oldObj))
				return
			}
			newPod, ok := newObj.(*corev1.Pod)
			if !ok {
				log.G(ctx).Error("UpdateFunc: newObj is not a *corev1.Pod", "type", fmt.Sprintf("%T", newObj))
				return
			}
...
func (pc *PodController) syncPodFromKubernetesHandler(ctx context.Context, key string) error {
	...
	if depth := pc.syncPodsFromKubernetes.Len(); depth > pc.podSyncWorkers*2 {
		...
		prev := pc.lastBackpressureLog.Load()
		if now-prev > int64(10*time.Second) {
			if pc.lastBackpressureLog.CompareAndSwap(prev, now) {
...
func (pc *PodController) syncPodInProvider(ctx context.Context, pod *corev1.Pod, key string) (retErr error) {
	...
	// more context is here: https://github.com/malformed-c/periapsis/pull/760
	...
	if pod.DeletionTimestamp != nil {
		...
		pc.deletePodsFromKubernetes.EnqueueWithoutRateLimitWithDelay(ctx, key, time.Second*time.Duration(gracePeriod))
		return nil
	}
