...
func (g *Gambit) GetPodStatus(ctx context.Context, namespace, name string) (*corev1.PodStatus, error) {
        ...
        // If the pod is in a terminal phase (set by Syzygy), return
        // the stored status directly. The systemd unit may already be cleaned
        // up (ResetUnit), so querying the runtime would give stale results.
        ...
        // Use the Runtime's MachineStatus for state lookups.
        // The old BatchWatcher.ContainerState cache no longer exists.
